...
export interface IResearchAreaPreviewItem extends TResearchAreaPreview {
	icon?: Icon;
}

export interface IResearchAreasPreviewProps {
	title?: string;
	description?: string;
	domains: IResearchAreaPreviewItem[];
	cta?: ReactNode;
	className?: string;
}
...
const DomainCell = ({
	...
	domain: IResearchAreaPreviewItem;
	index: number;
	size: "lg" | "sm";
}) => {
...
export const ResearchAreasPreview = ({
	title,
	description,
	domains,
	cta,
	className,
}: IResearchAreasPreviewProps) => {
	return (
		<section
			className={cn(
